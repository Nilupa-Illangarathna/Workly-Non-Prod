...
import { addPaymentMethod } from "@/actions/admin/payment-methods";

export default function PaymentMethodListDialog({ id }: { id: string }) {
  ...
  function onSubmit(values: z.infer<typeof paymentMethodSchema>) {
    startTransition(() => {
      addPaymentMethod(values, id).then((result) => {
        if (result.error) {
